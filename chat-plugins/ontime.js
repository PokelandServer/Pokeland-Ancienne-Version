...
exports.commands = {
	...
	ontime: function (target, room, user) {
		...
		if (!totalOntime) return this.sendReplyBox(hashColors(userid, true) + " has never been online on this server.");
		...
		if (isConnected) {
			this.sendReplyBox(hashColors(userid, true) + '\'s total ontime is <b>' + displayTime(convertTime(totalOntime)) + '</b>.' + ' Current ontime: <b>' + displayTime(convertTime((currentOntime))) + '</b>.');
		} else {
			this.sendReplyBox(hashColors(userid, true) + '\'s total ontime is <b>' + displayTime(convertTime(totalOntime)) + '</b>.' + ' Currently not online.');
		}
	},
