...
class Ambush {
	...
	updateJoins() {
		...
		if (this.players.size > 0) {
			msg += `<center><strong>${this.players.size}</strong> ${(this.players.size === 1 ? "user has" : "users have")} joined: ${Array.from(this.players).map(player => hashColors(player[0].name)).join(", ")}</center>`;
		}
	...
	leave(user, self) {
		...
		if (!this.round) {
			...
			this.room.add(`|html|${hashColors(user.name, true)} has left the game!`);
		}
	}
	...
	fire(user, target, self) {
		...
		this.madeMove = true;
		if (targetUser === user) {
			this.room.add(`|html|${hashColors(user.name, true)} shot themself!`);
		} else if (!this.players.get(targetUser).rounds) {
			this.room.add(`|html|${hashColors(user.name, true)} fired at ${hashColors(targetUser.name, true)}, but ${hashColors(targetUser.name, true)} has an active shield!`);
			return;
		} else {
			this.room.add(`|html|${hashColors(user.name, true)} fired at ${hashColors(targetUser.name, true)}!`);
		}
	...
	dq(target, self) {
		...
		this.room.add(`|html|${hashColors(targetUser.name, true)} has been disqualified from the game.`);
		if (this.checkWinner()) this.getWinner();
	}
	...
	getWinner() {
		...
		let msg = `|html|<div class = "infobox"><center>The winner of this game of Ambush is ${hashColors(winner, true)}! Congratulations!</center>`;
		if (this.room.isOfficial) {
			msg += `<center>${hashColors(winner, true)} has also won <strong>5</strong>for winning!</center>`;
		...
		if (user) {
			let msg = `<div class = "infobox"><center>This game of Ambush has been forcibly ended by ${hashColors(user.name, true)}.</center></div>`;
			if (!this.madeMove) {
...
let commands = {
	...
	proceed: function (target, room, user) {
		...
		room.add(`|html|(${hashColors(user.name, true, true)} forcibly started round 1.)`);
