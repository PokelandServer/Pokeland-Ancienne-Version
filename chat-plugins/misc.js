...
function clearRoom(room) {
	...
	for (let u in room.users) {
		if (room.users[u].userid === 'distrib') continue;
		users.push(u);
		let actuser = Users.get(room.users[u]);
		actuser.leaveRoom(room, actuser.connections[0]);
	}
	len = users.length;
	setTimeout(function () {
		while (len--) {
	...
	authority: function (target, room, user, connection) {
		...
		for (let u in Users.usergroups) {
			...
			if (ranks.includes(rank)) {
				...
				if (name) rankLists[rank].push(hashColors(name, (Users(name) && Users(name).connected)));
			}
		}
...
/* 
	...
	*/
	...
	regdatehelp: ["/regdate - Please specify a valid username."], // poce blo
