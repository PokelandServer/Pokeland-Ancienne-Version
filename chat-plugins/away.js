...
exports.commands = {
	away: function (target, room, user) {
		...
		newName += ' - ' + status;
		if (newName.length > 28) return this.sendReply("\"" + target + "\" is too long to use as your away status.");
	...
	forceback: function (target, room, user) {
		if (!target) return this.errorReply('/forceback user');
		if (!this.canTalk()) return;

		target = this.splitTarget(target);
		let targetUser = this.targetUser;
		if (!this.can('bans', targetUser)) return false;
		if (!targetUser) return this.errorReply(`Utilisateur ${this.targetUsername} introuvable.`);
		if (!targetUser.isAway) return this.errorReply(`L'Utilisateur ${this.targetUsername} n'est pas marqué comme absent.`);
		...
		let status = parseStatus(newName.substr(statusIdx + 3), false);
		newName = newName.substr(0, statusIdx);
		targetUser.forceRename(newName, targetUser.registered);
